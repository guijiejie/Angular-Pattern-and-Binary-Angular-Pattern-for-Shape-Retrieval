function [ LbapH ] = LBAP12P( Cs, dp1,dp2)
% dp1 是局部角度计算的半径
% dp2 是局部角度比较的半径
...
%
Lbap = zeros(length(Cs),12);

%
for i = dp2+1:length(LapE)-dp2
    % 取出13点的Lap
    TAs = LapE([i-dp2 i-dp2*(5/6) i-dp2*(4/6) i-dp2*(3/6) i-dp2*(2/6) i-dp2*(1/6) i i+dp2*(1/6) i+dp2*(2/6) i+dp2*(3/6) i+dp2*(4/6) i+dp2*(5/6) i+dp2]);
    %
    if TAs(1)>=TAs(7)
        %
        Lbap(i-dp2,1) = 1;     
...
Lbap12 = Lbap(:,12)*2048;
%
LbapV = Lbap1+Lbap2+Lbap3+Lbap4+Lbap5+Lbap6+Lbap7+Lbap8+Lbap9+Lbap10+Lbap11+Lbap12;
%
for i = 1:length(Cs)
    %
