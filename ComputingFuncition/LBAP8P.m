function [ LbapH ] = LBAP8P( Cs, dp1,dp2)
% dp1 是局部角度计算的半径
% dp2 是局部角度比较的半径
...
%
Lbap = zeros(length(Cs),8);

%
for i = dp2+1:length(LapE)-dp2
    % 取出9点的Lap
    TAs = LapE([i-dp2 i-dp2*(3/4) i-dp2*(2/4) i-dp2*(1/4) i i+dp2*(1/4) i+dp2*(2/4) i+dp2*(3/4) i+dp2]);
    %
    if TAs(1)>=TAs(5)
        %
        Lbap(i-dp2,1) = 1;     
...
Lbap8 = Lbap(:,8)*1;
%
LbapV = Lbap1+Lbap2+Lbap3+Lbap4+Lbap5+Lbap6+Lbap7+Lbap8;
%
for i = 1:length(Cs)
    %
